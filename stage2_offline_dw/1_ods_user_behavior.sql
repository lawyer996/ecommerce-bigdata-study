...
--
-- 数据源：dataset/UserBehavior.csv（天池真实数据，behavior_type 取值 pv/fav/cart/buy）
-- 本地等价实现见同目录 run_local_dw.py（DuckDB，无需 Hive 集群也能跑通同样逻辑）
-- ============================================================
...
CREATE EXTERNAL TABLE ods_user_behavior (
    ...
    behavior_type string,   -- 行为类型：pv浏览 fav收藏 cart加购 buy购买
...
-- load data local inpath '/opt/data/UserBehavior.csv' into table ods_user_behavior;
