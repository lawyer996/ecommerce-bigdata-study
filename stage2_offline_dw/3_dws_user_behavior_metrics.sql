...
-- 核心思想：聚合计算 PV/UV/转化率，生成指标宽表，供报表/BI直接取数
...
CREATE EXTERNAL TABLE dws_user_behavior_metrics (
    ...
    buy_cnt       bigint,      -- 购买次数
    cart_rate     decimal(10,4),  -- 浏览→加购转化率
    buy_rate      decimal(10,4),  -- 浏览→购买转化率
...
SELECT
    ...
    -- 转化率：加购/浏览、购买/浏览、购买/加购
    CAST(SUM(CASE WHEN behavior_type = 3 THEN 1 ELSE 0 END) / SUM(CASE WHEN behavior_type = 1 THEN 1 ELSE 0 END) AS decimal(10,4)) AS cart_rate,
    CAST(SUM(CASE WHEN behavior_type = 4 THEN 1 ELSE 0 END) / SUM(CASE WHEN behavior_type = 1 THEN 1 ELSE 0 END) AS decimal(10,4)) AS buy_rate,
    CAST(SUM(CASE WHEN behavior_type = 4 THEN 1 ELSE 0 END) / SUM(CASE WHEN behavior_type = 3 THEN 1 ELSE 0 END) AS decimal(10,4)) AS cart2buy_rate
